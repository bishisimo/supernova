...
func NewOption() *ConfigOption {
	return &ConfigOption{
		ConnectOption: new(ConnectOption),
		QueryOption:   new(QueryOption),
		StoreOption:   new(StoreOption),
...
func NewConnectOptionWithFlag() *ConnectOption {
	result := new(ConnectOption)
	flag.StringVar(&result.DriveName, "drive", "", "数据库类型")
	flag.StringVar(&result.UserName, "user", "", "数据库用户名")
	flag.StringVar(&result.Password, "psw", "", "数据库密码")
	flag.StringVar(&result.Ip, "ip", "", "数据库ip地址")
	flag.UintVar(&result.Port, "port", 0, "数据库端口,默认使用数据库的默认端口")
	flag.StringVar(&result.DbName, "db", "", "数据库名")
	//result := &ConnectOption{
	//	DriveName: flag.String("drive", "", "数据库类型"),
	//	UserName:  flag.String("user", "", "数据库用户名"),
	//	Password:  flag.String("psw", "", "数据库密码"),
...
func NewQueryOptionWithFlag() *QueryOption {
	result := new(QueryOption)
	flag.StringVar(&result.TableName, "table", "", "数据表名(必填)")
	flag.StringVar(&result.FieldSelect, "select", "", "需求字段(用','分割)")
	flag.StringVar(&result.FieldFlag, "field", "", "增量汇集数据的字段(当需要增量更新时必须指定)")
	flag.StringVar(&result.StartFlag, "start", "", "增量汇集的标记起始值(可选类型为数字与时间)")
	flag.StringVar(&result.UnitFlag, "unit", "", "增量汇集定时单位(支持选项:hour,day,month,year)")
	flag.StringVar(&result.QueryContext, "query", "", "自定义查询sql")
	flag.BoolVar(&result.IsStartContain, "isStartContain", false, "是否包含增量汇集起始数据")
...
func NewStoreOptionWithFlag() *StoreOption {
	result := new(StoreOption)
	flag.StringVar(&result.DestType, "destType", "", "目标路径类型")
	flag.StringVar(&result.DestPath, "destPath", "", "目标路径")
	flag.BoolVar(&result.IsRetainLocal, "isRetainLocal", false, "是否保留本地临时文件(默认不保存)")
	//result := &StoreOption{
...
func NewS3OptionWithFlag() *S3Option {
	result := new(S3Option)
	flag.StringVar(&result.S3Id, "s3Id", "", "s3的accessId")
	flag.StringVar(&result.S3Secret, "s3Secret", "", "s3的accessSecret")
	flag.StringVar(&result.S3Endpoint, "s3Endpoint", "", "s3的endpoint")
	flag.StringVar(&result.S3Region, "s3Region", "", "s3的region")
	flag.StringVar(&result.S3Bucket, "s3Bucket", "", "s3的bucket")
